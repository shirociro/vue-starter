    ...
    >
      <div class="p-4">
        ...
        <div v-else class="space-y-4">
          ...
          >
            <template
              v-if="error && error.includes('Title')"
              #validationMessage
            >
              {{ error }}
            </template>
          ...
          <fwb-input
            ...
            size="sm"
            :validation-status="
              error && (error.includes('Description') || error.includes('exists'))
                ? 'error'
                : ''
            "
          >
...
const props = defineProps({
  existingTasks: { type: Array, required: true },
});
const emit = defineEmits(["add"]) // define 'add' event
...
const validate = () => {
  ...
  if (props.existingTasks.some((u) => u.title === title.value.trim())) {
    error.value = "Title already exists";
    return false;
  }
